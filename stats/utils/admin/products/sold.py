...
def get_yearly_sold_products():
    today = datetime.date.today()
    earliest_year_item = (
        OrderItem.objects.filter(created__isnull=False).order_by("created").first()
    )

    if not earliest_year_item:
        return {"years": [], "sold_products": []}

    earliest_year = earliest_year_item.created.year
